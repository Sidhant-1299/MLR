...
import pandas as pd
import numpy as np
from MLR.mlr_wrapper import MLRWrapper  # Update this if the class is named differently
...
#     assert np.testing.assert_allclose(predictions.flatten(), y, rtol=1e-4, atol=1e-4)
...
def test_get_coefficients_matches_expected():
    ...
    model = MLRWrapper(df, target_col="y")
    model.fit()
    coeffs = model.get_coefficients().reshape(-1,1) 

    # Check coefficients (intercept, x1, x2)
    # assert coeffs.shape == (3,1)
    print("Coeffs: ")
    print(coeffs)
    assert np.allclose(coeffs,[2,3,4],atol=1e-4)